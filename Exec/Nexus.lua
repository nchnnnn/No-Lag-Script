...
local Nexus = {}
-- Attempt to find the WebSocket connection function based on the executor
...
-- If no WebSocket connection function is found, show an error message
if not WSConnect then
    if messagebox then
        -- Show a messagebox if it's available (or you could print to the console)
        messagebox(('Nexus encountered an error while launching!\n\n%s'):format(
            'Your exploit (' .. (identifyexecutor and identifyexecutor() or 'UNKNOWN') .. ') is not supported or WebSocket is unavailable.'),
            'Roblox Account Manager', 0)
    end
    return  -- Exit the script since WebSocket functionality is necessary for the script to work
end

-- Continue with the rest of the code if WebSocket is available
print("WebSocket connection available. Proceeding...")
